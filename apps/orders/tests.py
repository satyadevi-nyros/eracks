...
class FixturesTest(TestCase):
    fixtures = ['orders.yaml']  # nested orders dump includes user, userena, customer - JJW no need for: 'users.yaml', 'customers.yaml', 'catax.yaml',

    def test_orders_fixtures(self):
        order1 = Order.objects.get(pk=54876)
        self.assertTrue(order1)

class ViewsTests(TestCase):
    fixtures = ['catax.yaml', 'products.yaml', 'orders.yaml', 'quickpages.yaml'] # 'users.yaml', 'customers.yaml',
